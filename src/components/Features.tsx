...
const FeatureCard: React.FC<FeatureCardProps> = ({ 
  ...
  return (
    <ScrollAnimatedElement
      className={`group relative backdrop-blur-sm bg-gray-800 bg-opacity-30 border border-gray-700 rounded-xl p-6 transition-all duration-500 ease-out hover:border-blue-500 hover:scale-[1.02] hover:shadow-2xl hover:shadow-blue-500/10 ${link ? 'cursor-pointer' : ''} ${isEcosystem ? 'min-h-[280px]' : ''} h-full flex flex-col`}
      animation="fadeUp"
      delay={delay}
    >
      ...
      <div className="relative z-10 flex-1 flex flex-col items-center text-center">
        ...
        <p className="text-gray-400 group-hover:text-gray-300 transition-colors duration-300 mb-6 flex-grow">{description}</p>
        ...
        <div className="flex flex-wrap justify-center gap-2 mt-auto">
          {(onSpendClick || spendOptions.length > 0) && (
            <button
              ...
              className={`flex items-center px-3 py-2 rounded-lg text-sm transition-all duration-300 hover:scale-105 ${
                activeTab === 'spend'
          ...
          {title === "AIRE Social" && (
            <button
              ...
              className="px-4 py-2 bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 rounded-lg text-white transition-all duration-300 hover:scale-105"
            >
              Explore Marketplace
            </button>
          )}
...
const Features = () => {
  ...
  const ecosystemFeatures = [
    ...
    },
    {
      ...
      spendOptions: [
        "🛒 Buy products with AIRE discount",
        "📢 Promote via AIRE Ads",
        "🎨 Professional AI store design",
        "🔐 Premium seller subscription",
        "🚚 Logistics and shipping payments",
        "🏷️ NFT certificates of authenticity",
        "🧑‍🏫 Hire creator ambassadors",
        "📊 Access DAO market research",
        "🌍 Expand to new countries",
        "💬 AI-assisted customer service"
      ],
      earnOptions: [
        "📦 Sell physical or digital products — +100 $AIRE/Sale",
        "💸 Authorized dropshipping — +% over automatic sales",
        "🛍️ Load your entire store — +10 $AIRE per 5 stars",
        "🧾 Receive positive feedback — +10% commissions on $AIRE",
        "🤝 Affiliate with creators — +0.1 $AIRE per interaction",
        "🎯 Featured products with clicks — +15 $AIRE per conversion",
        "📲 Sale with referral links — +0.5 $AIRE per view",
        "📢 Promote on external networks — +5% extra revenue in $AIRE",
        "🧠 Activate smart selling — +25 $AIRE per unit (DAO approved)",
  ...
  return (
    <section id="features" className="py-24 relative overflow-hidden">
      {/* All overlay modals */}
      {showEarningsOverlay && <EarningsOverlay onClose={() => setShowEarningsOverlay(false)} />}
      {showSpendOverlay && <SpendOverlay onClose={() => setShowSpendOverlay(false)} />}
      {showMarketplaceEarningsOverlay && <MarketplaceEarningsOverlay onClose={() => setShowMarketplaceEarningsOverlay(false)} />}
      {showMarketplaceSpendOverlay && <MarketplaceSpendOverlay onClose={() => setShowMarketplaceSpendOverlay(false)} />}
      {showStudioEarningsOverlay && <StudioEarningsOverlay onClose={() => setShowStudioEarningsOverlay(false)} />}
      {showStudioSpendOverlay && <StudioSpendOverlay onClose={() => setShowStudioSpendOverlay(false)} />}
      {showGamersEarningsOverlay && <GamersEarningsOverlay onClose={() => setShowGamersEarningsOverlay(false)} />}
      {showGamersSpendOverlay && <GamersSpendOverlay onClose={() => setShowGamersSpendOverlay(false)} />}
      {showWalletEarningsOverlay && <WalletEarningsOverlay onClose={() => setShowWalletEarningsOverlay(false)} />}
      {showWalletSpendOverlay && <WalletSpendOverlay onClose={() => setShowWalletSpendOverlay(false)} />}
      {showLifeEarningsOverlay && <LifeEarningsOverlay onClose={() => setShowLifeEarningsOverlay(false)} />}
      {showLifeSpendOverlay && <LifeSpendOverlay onClose={() => setShowLifeSpendOverlay(false)} />}
      {showDaoEarningsOverlay && <DaoEarningsOverlay onClose={() => setShowDaoEarningsOverlay(false)} />}
      {showDaoSpendOverlay && <DaoSpendOverlay onClose={() => setShowDaoSpendOverlay(false)} />}
      {showMusicEarningsOverlay && <MusicEarningsOverlay onClose={() => setShowMusicEarningsOverlay(false)} />}
      {showMusicSpendOverlay && <MusicSpendOverlay onClose={() => setShowMusicSpendOverlay(false)} />}
      {showPlayEarningsOverlay && <PlayEarningsOverlay onClose={() => setShowPlayEarningsOverlay(false)} />}
      {showPlaySpendOverlay && <PlaySpendOverlay onClose={() => setShowPlaySpendOverlay(false)} />}
      {showBusinessEarningsOverlay && <BusinessEarningsOverlay onClose={() => setShowBusinessEarningsOverlay(false)} />}
      {showBusinessSpendOverlay && <BusinessSpendOverlay onClose={() => setShowBusinessSpendOverlay(false)} />}
      {showAdsEarningsOverlay && <AdsEarningsOverlay onClose={() => setShowAdsEarningsOverlay(false)} />}
      {showAdsSpendOverlay && <AdsSpendOverlay onClose={() => setShowAdsSpendOverlay(false)} />}
      {showSalesEarningsOverlay && <SalesEarningsOverlay onClose={() => setShowSalesEarningsOverlay(false)} />}
      {showSalesSpendOverlay && <SalesSpendOverlay onClose={() => setShowSalesSpendOverlay(false)} />}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-20">
          {ecosystemFeatures.map((feature, index) => (
            <FeatureCard
              ...
              spendOptions={feature.spendOptions}
              earnOptions={feature.earnOptions}
              onEarnClick={feature.onEarnClick}
              onSpendClick={feature.onSpendClick}
              link={feature.link}
              delay={index * 100}
              isEcosystem={true}
            />
